...
- **Arcane loop is PI-derived and already OpenAI-compatible.** `vendor/agent-loop.ts` runs the loop; `vendor/agent.ts` wraps it with a **pluggable `streamFn`**; `arcane-stream.ts` POSTs OpenAI-format `chat/completions` (tools converted to `function` format, `tool_calls` parsed) to `api.unityide.app`. **Swapping models = change base URL + key + model name + settings UI.** No loop rewrite. Tool-calling is already OpenAI-style, not Anthropic `tool_use`.
