...
export const SETTING_DEFINITIONS: SettingDefinition[] = [
  ...
  { key: 'editor.formatOnSave', type: 'boolean', category: 'Editor', label: 'Format On Save', description: 'Run the language server\u2019s formatter on a file each time it is saved. A formatting failure never blocks the save.' },
  ...
  { key: 'ai.specialists.enabled', type: 'boolean', category: 'AI', label: 'Unity Specialist Agents', description: 'Use focused agents, saved scene authoring, and input-driven gameplay verification. Requires integration package 0.3.0. Experimental until live acceptance checks pass.' },
  ...
  { key: 'lsp.csharp.analyzers', type: 'boolean', category: 'Unity', label: 'Roslyn Analyzers (Unity)', description: 'Run the official Microsoft.Unity.Analyzers inside the C# language server: 43 Unity inspections plus the suppressors that stop [SerializeField] fields being reported as unused. Costs a full-project analyzer pass per diagnostics refresh. Per-rule severities can be tuned in a .editorconfig (dotnet_diagnostic.UNT0001.severity).' },
  ...
  { key: 'unity.assetGate.enabled', type: 'boolean', category: 'Unity', label: 'AI Unity Asset Verification', description: "After the AI writes a .uxml, .uss, .inputactions or .asset, check it and feed problems back so it self-corrects: malformed UXML, USS properties Unity silently ignores, an .inputactions that no longer parses, a broken script link on an asset." },
  ...
  { key: 'unity.consoleCheck.enabled', type: 'boolean', category: 'Unity', label: 'AI Console Check', description: 'After an agent turn, look for new console errors, failed tests and compiler errors, and let the AI make one repair pass.' },
  { key: 'unity.consoleCheck.autoRepair', type: 'boolean', category: 'Unity', label: 'AI Console Auto-Repair', description: 'Let the AI make one repair pass when the console check finds new problems.' },
  ...
  { key: 'unity.uiToolkit.panel', type: 'boolean', category: 'Unity', label: 'Unity UI Panel', description: 'Show the Unity UI panel: every .uxml and .uss in the project, with the classes no stylesheet declares and the properties Unity drops at import.' },
  ...
  { key: 'unity.scriptableObjects.browser', type: 'boolean', category: 'Unity', label: 'ScriptableObjects Panel', description: 'Show the ScriptableObjects browser in the activity bar, grouping every data asset by its type.' },
