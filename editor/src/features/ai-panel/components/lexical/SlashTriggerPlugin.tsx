 ...
 * the SlashCommandPopover with the Claude bridge's advertised commands. Active
 * only for the Claude agent. Slash commands send as ordinary prompt text
 * (the bridge interprets them), so picking just rewrites the editor to `/<name> `.
 *
...
function SlashTriggerPlugin() {
  ...
  const commands = useAiStore((s) => s.claudeAvailableCommands);
  const [trigger, setTrigger] = useState<TriggerState | null>(null);

  const detect = useCallback(() => {
    if (selectedAgent !== 'claude' || commands.length === 0) {
