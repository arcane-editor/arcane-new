...
describe('composerPlaceholder — Arcane', () => {
  it('keeps the per-mode copy the Arcane loop actually honours', () => {
...
describe('composerPlaceholder — external agent', () => {
   ...
   * "⏎ to plan", because the placeholder was driven by Arcane's `mode`. Claude
   ...
   */
  it('never promises Arcane plan-mode behaviour', () => {
  ...
  it('is the same regardless of the Arcane mode left behind', () => {
    const texts = new Set(
...
describe('composerPlaceholder — pending question', () => {
  it('outranks mode and agent alike', () => {
    for (const agent of ['arcane', 'claude'] as const) {
