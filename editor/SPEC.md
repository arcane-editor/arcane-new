...
- **Bridge transport is a Unix domain socket, NOT a WebSocket.** Socket path resolved via discovery file `<project>/Library/ArcaneIDE/bridge.json` (`{socketPath, protocolVersion, ideVersion, idePid}`); frames are **4-byte big-endian length prefix + UTF-8 JSON**, 16 MB cap. RPC is request/response-correlated by `id` through the `unity_ipc_request` Tauri command (10 s timeout). Rust: `src-tauri/src/unity_ipc.rs`. *(Windows named-pipe/TCP transport + per-connection auth token are deferred — flagged in code.)*
- **The Unity bridge C# code is part of the single Unity extension package**, which lives in the sibling `arcane` repo at `arcane/arcane-extension/` (`com.arcane.editor`, Editor-only) — the single source of truth. It merges the live socket bridge (`Arcane.Bridge`) with the external-code-editor integration (`Arcane.Editor`: External Tools registration, double-click open, `.sln`/`.csproj` generation). `scripts/sync-unity-bridge.mjs` copies it into a gitignored `unity-bridge/` staging folder before every `tauri dev`/`build` (wired via `beforeDevCommand`/`beforeBuildCommand`); Tauri bundles that as a resource and `unity_install_bridge` installs it into `<project>/Packages/com.arcane.editor/` (embedded package; no manifest.json surgery). It has NOT been run against a live Unity Editor yet — see "live validation" below.
...
| **F-5.6** safety & approval | ✅ | Engine-mutate tools block on per-action inline approval (`approval-gate.ts`, reuses `useAiStore` permission requests; sequential loop ⇒ never batched). `PermissionRequestBlock` routes to Claude(ACP)/Arcane by `selectedAgent`. |
...
2. **F-10 T10.1** external-editor registration + `arcane://` deep link — the one remaining buildable feature; needs a Tauri deep-link plugin + OS scheme registration that only functions in a packaged/installed build (not testable headlessly), so it was deferred.
