...
7. **macOS `tauri dev` fallback.** `bunx tauri dev` → Account tab defaults to the device-code UI (unbundled macOS can't register the scheme); device flow completes against `dev.arcaneai.org/auth/device`.
   - **Windows/Linux dev footgun (noted):** plain `bunx tauri dev` on Win/Linux runs `register_all()` with the BASE config, so it registers the prod `arcane://` scheme and would hijack it from an installed prod Arcane on that dev machine until prod re-registers. For deep-link testing on Win/Linux dev, run `bunx tauri dev --config src-tauri/tauri.dev.conf.json` so it registers `arcane-dev://` instead.
